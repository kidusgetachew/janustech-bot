...
Mobile responsive + WebSocket real-time price updates
"""
...
@media (max-width: 768px) {
    ...
    .janustech-title { font-size: 16px !important; letter-spacing: 2px !important; }
    .janustech-subtitle { display: none !important; }
    /* Full width cards */
    .card { padding: 12px !important; margin-bottom: 8px !important; }
    /* Hide watchlist on mobile by default */
...
defaults = {
    ...
    "live_prices": {}, "last_price_update": 0,
}
...
def fetch_live_prices(bot, symbols):
    """Fetch latest prices for ticker display."""
    prices = {}
    if not bot or not bot.client or not bot.client.connected:
        return prices
    try:
        for sym in symbols[:6]:  # limit to 6 for performance
            df = bot.client.get_bars(sym, "5Min", lookback_days=2)
            if not df.empty and len(df) >= 2:
                last = df["close"].iloc[-1]
                prev = df["close"].iloc[-2]
                change_pct = ((last - prev) / prev) * 100
                prices[sym] = {"price": last, "change_pct": change_pct}
    except Exception:
        pass
    return prices
...
with col_left:
    if st.session_state.bot and st.session_state.bot.is_running:
        st.markdown('<span class="pill pill-green"><span class="live-dot">●</span> LIVE</span>', unsafe_allow_html=True)
...
with col_center:
    ...
    <div style='text-align:center;padding:4px 0;'>
        <div class='janustech-title' style='font-family:Rajdhani,sans-serif;font-size:22px;font-weight:700;letter-spacing:4px;text-transform:uppercase;color:#e6edf3;'>PROJECT JANUSTECH</div>
        <div class='janustech-subtitle' style='font-family:JetBrains Mono,monospace;font-size:9px;letter-spacing:3px;color:#484f58;text-transform:uppercase;'>Smart Money Concepts Trading Engine</div>
    </div>
    """, unsafe_allow_html=True)
with col_right:
...
# ── LIVE PRICE TICKER ──────────────────────────────────────────────────────────
if st.session_state.connected and st.session_state.bot:
    now = time.time()
    # Update prices every 60 seconds
    if now - st.session_state.last_price_update > 60:
        st.session_state.live_prices = fetch_live_prices(
            st.session_state.bot,
            st.session_state.config.markets.symbols
        )
        st.session_state.last_price_update = now

    if st.session_state.live_prices:
        ticker_html = '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px;overflow-x:auto;">'
        for sym, data in st.session_state.live_prices.items():
            price = data["price"]
            chg = data["change_pct"]
            color = "price-up" if chg >= 0 else "price-down"
            arrow = "▲" if chg >= 0 else "▼"
            ticker_html += f'<span class="price-ticker"><span style="color:#e6edf3;font-weight:600;">{sym}</span><span class="{color}">${price:,.2f} {arrow}{abs(chg):.2f}%</span></span>'
        ticker_html += '</div>'
...
if page == "Setup":
    ...
    with col1:
        ...
        if not paper:
            st.markdown('<div style="padding:10px;background:rgba(248,81,73,0.08);border:1px solid rgba(248,81,73,0.3);border-radius:6px;margin-bottom:12px;font-size:13px;color:#f85149;">⚠️ WARNING: Live Trading uses REAL money. Make sure you have tested on paper trading first.</div>', unsafe_allow_html=True)
...
# ── TERMINAL ───────────────────────────────────────────────────────────────────
    ...
    # On mobile hide watchlist, show chart + analysis only
    ...
    with cat_col:
        ...
        for group, symbols in STOCK_CATALOG.items():
            ...
            for sym in filtered:
                ...
                # Show live price next to symbol if available
                live = st.session_state.live_prices.get(sym, {})
                btn_label = sym
                if live:
                    chg = live.get("change_pct", 0)
                    arrow = "▲" if chg >= 0 else "▼"
                    btn_label = f"{sym} {arrow}{abs(chg):.1f}%"

                if st.button(btn_label, key=f"wl_{sym}", use_container_width=True):
    ...
    with chart_col:
        ...
        # Show live price from ticker if available
        live_data = st.session_state.live_prices.get(symbol, {})
        if live_data:
            price = live_data.get("price", price)
            change_pct = live_data.get("change_pct", change_pct)
    ...
    with analysis_col:
        ...
        if smc_data and show_smc:
            ...
            tooltips = {
                "daily_bias": "Bullish/bearish based on higher timeframe structure",
                "liquidity_sweep": "Price grabbed stops then reversed — smart money trap",
                "bos": "Break of Structure — confirms new market direction",
                "price_zone": "Price in discount zone (buy) or premium zone (sell)",
                "fvg": "Fair Value Gap — 3-candle imbalance, price likely returns here",
                "order_block": "Last opposing candle before the big move — institutional entry"
            }
            ...
            for label, key in items:
                ...
                st.markdown(f'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #21262d;" title="{tip}"><span style="color:{color};font-size:12px;font-family:JetBrains Mono,monospace;" title="{tip}">{"●" if active else "○"} {label}</span><span style="color:{"#3fb950" if active else "#21262d"};font-size:10px;">{"✓" if active else "—"}</span></div>', unsafe_allow_html=True)
...
# ── DASHBOARD ──────────────────────────────────────────────────────────────────
    ...
    if st.session_state.bot and st.session_state.connected:
        ...
        if dd.get("is_killed"):
            ...
            st.markdown(f'<div style="padding:12px;background:rgba(210,153,34,0.1);border:1px solid rgba(210,153,34,0.3);border-radius:6px;color:#d29922;font-size:13px;margin-bottom:12px;">⚠ Daily drawdown at {dd.get("daily_drawdown_pct",0):.1f}%</div>', unsafe_allow_html=True)
    else:
...
# ── STRATEGY ───────────────────────────────────────────────────────────────────
    ...
    with tab2:
        ...
        with col1:
            ...
            trail_pct = st.number_input("Trail Distance (%)", value=1.0, min_value=0.1, max_value=5.0, step=0.1)
        with col2:
            activate_r = st.number_input("Activate at R-Multiple", value=1.0, min_value=0.5, max_value=3.0, step=0.5)
        if st.button("Save Trailing Stop", use_container_width=True, key="save_trail"):
            st.success(f"Trailing stop {'enabled' if trail_enabled else 'disabled'}")
        st.markdown('</div>', unsafe_allow_html=True)
    with tab3:
        ...
        nf = st.number_input("FVG Min Size (pts)", value=strat.fvg_min_size_pts, min_value=1.0, max_value=50.0, step=1.0)
        no = st.number_input("OB Lookback (candles)", value=float(strat.ob_lookback), min_value=5.0, max_value=50.0, step=1.0)
        nc = st.number_input("Min Confluence Score (/6)", value=float(strat.confluence_min), min_value=1.0, max_value=6.0, step=1.0)
    ...
    with tab4:
        ...
        with col1:
            ...
            si = st.text_input("Symbols (comma separated)", value=", ".join(st.session_state.config.markets.symbols))
